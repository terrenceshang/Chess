public class Queen extends Piece {
    ...
    private boolean validMove(Board board, Spot start, Spot end) throws Exception {
        ...
        if ( (distX*distY==0) || (distX/distY==1) ) {
    ...
    private boolean isBlocked (Board board, Spot start, Spot end) throws Exception { //Check if there are any blockers
        ...
        else if (xSign != 0) { //if the move is horizontal
            for (int i = 0; i < distX; i++) {
                Spot tempSpot = board.getBox(start.getX()+xSign*i, start.getY());
                if (tempSpot.getPiece().getIsWhite() == null) {
                    continue;
                } else {
                    return true;
        ...
        else if (ySign != 0){ //if the move is vertical
            for (int i = 0; i < distY; i++) {
                Spot tempSpot = board.getBox(start.getX(), start.getY()+ySign*i);
                if (tempSpot.getPiece().getIsWhite() == null) {
                    continue;
                } else {
                    return true;
